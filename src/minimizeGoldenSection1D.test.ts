...
describe('minimize',  () => {
  test('Minimizes 1 / (x - 1) in [0, 2]', () => {
    let status: GoldenSectionMinimizeStatus = {};
    assertAlmostEqual(minimizeGoldenSection1D(function (x) { return 1 / (x - 1); }, {
      lowerBound: 0,
      upperBound: 2
    }, status), 1);
  ...
  test('Bails out on unbounded minimization of -x^2', () => {
    let status: GoldenSectionMinimizeStatus = {};
    minimizeGoldenSection1D(function (x) {
      return -x * x;
    }, undefined, status);
  ...
  test('Succeeds out on bounded minimization of -x^2', () => {
    let status: GoldenSectionMinimizeStatus = {};
    let answer = minimizeGoldenSection1D(function (x) {
      return -x * x;
  ...
  test('Minimizes sqrt(|x|) in (-inf, inf)', () => {
    assertAlmostEqual(minimizeGoldenSection1D(function (x) { return Math.sqrt(Math.abs(x)); }), 0);
    
  });

  test('returns answer if tolerance not met', () => {
    let status: GoldenSectionMinimizeStatus = {};
    expect(isNaN(minimizeGoldenSection1D(function (x) { return x * (x - 2); }, {tolerance: 0, maxIterations: 200}, status))).toBeTruthy();
  ...
  test('minimizes a x(x-2) in (-inf, inf)', () => {
    assertAlmostEqual(minimizeGoldenSection1D(function (x) { return x * (x - 2); }), 1);
    
  });

  test('minimizes x(x-2) in [-6, inf)', () => {
    assertAlmostEqual(minimizeGoldenSection1D(function (x) { return x * (x - 2); }, {
      lowerBound: -6
    }), 1);
  ...
  test('minimizes a cubic', () => {
    assertAlmostEqual(minimizeGoldenSection1D(function (x) { return x * (x - 2) * (x - 1); }, {
      lowerBound: 0,
      upperBound: 3
    }), (3 + Math.sqrt(3)) / 3);
  ...
  test('fails to minimize a cubic with no bounds', () => {
    expect(isNaN(minimizeGoldenSection1D(function (x) { return x * (x - 2) * (x - 1); }))).toBeTruthy();
  ...
  test('minimizes a line', () => {
    assertAlmostEqual(minimizeGoldenSection1D(function (x) { return x; }, {
      lowerBound: 0.5,
      upperBound: 1
    }), 0.5);
  ...
  test('minimizes a cusp', () => {
    assertAlmostEqual(minimizeGoldenSection1D(function (x) { return Math.sqrt(Math.abs(x - 5)); }), 5);
