import { GoldenSectionMinimizeStatus } from "./goldenSectionMinimize";
...
describe("minimizeGoldenSection1D", () => {
  ...
  test("Succeeds out on bounded minimization of -x^2", async () => {
    const status: GoldenSectionMinimizeStatus = {};
    const answer = await minimizeGoldenSection1D(
      (x) => {
        return -x * x;
      },
  ...
  test("Minimizes sqrt(x) in [0, inf)", async () => {
    assertAlmostEqual(
      await minimizeGoldenSection1D(Math.sqrt, {
        lowerBound: 0,
      }),
      0,
    );
  ...
  test("Minimizes sqrt(|x|) in (-inf, inf)", async () => {
    assertAlmostEqual(
      await minimizeGoldenSection1D((x) => Math.sqrt(Math.abs(x))),
      0,
    );
  });
  ...
  test("minimizes x(x-2) in [-6, inf)", async () => {
    assertAlmostEqual(
      await minimizeGoldenSection1D((x) => x * (x - 2), {
        lowerBound: -6,
      }),
      1,
    );
  ...
  test("minimizes a cubic", async () => {
    assertAlmostEqual(
      await minimizeGoldenSection1D((x) => x * (x - 2) * (x - 1), {
        lowerBound: 0,
        upperBound: 3,
      }),
      (3 + Math.sqrt(3)) / 3,
  ...
  test("fails to minimize a cubic with no bounds", async () => {
    expect(
      isNaN((await minimizeGoldenSection1D((x) => x * (x - 2) * (x - 1)))!),
    ).toBeTruthy();
  });
  ...
  test("minimizes cosine", async () => {
    assertAlmostEqual(
      await minimizeGoldenSection1D(Math.cos, { lowerBound: 0, upperBound: 1 }),
      1,
    );
  });
  ...
  test("minimizes cosine", async () => {
    assertAlmostEqual(
      await minimizeGoldenSection1D(Math.cos, { guess: -3 }),
      -Math.PI,
    );
  });
  ...
  test("minimizes a line", async () => {
    assertAlmostEqual(
      await minimizeGoldenSection1D((x) => x, {
        lowerBound: 0.5,
        upperBound: 1,
      }),
      0.5,
  ...
  test("minimizes a cusp", async () => {
    assertAlmostEqual(
      await minimizeGoldenSection1D((x) => Math.sqrt(Math.abs(x - 5))),
      5,
    );
  });
});
